...
class MangaDexCog(
    ...
    @discord.app_commands.command(
        description="Add a manga to follow the latest chapters of."
    )
    ...
    async def follow(
        self, interaction: discord.Interaction, manga: str, channel: discord.TextChannel
    ):
        """
    ...
    async def notifications(self, interaction: discord.Interaction):
        ...
        with Session(expire_on_commit=False) as db:
            mangas = (
                db.query(Manga).filter(Manga.guild_id == interaction.guild.id).all()
            )
    ...
    async def post(self, manga: Manga | None, latest: Chapter):
        ...
        if manga.cover is not None:
            url = (
                f"https://uploads.mangadex.org/covers/{manga.mangadex_id}/{manga.cover}"
            )
